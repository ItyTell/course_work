...
float** model1(int size, float beta, float beta1, float l, float p1, float p2, float a, 
    ...
    float* deaths = malloc((int)(size*dt) * sizeof(float));
    ...
    for (int i = 0; i < size - 1; i++){
        ...
        P[i + 1] = P[i] + a * p2 * E[i] * dt - (gama_a + gama_i) * P[i] * dt- sigma_p * P[i] * dt;
        ...
        sum += a * p1 * E[i] * dt + a * p2 * P[i] * dt + gama_a * (I[i] + P[i]) * dt;
        sum2 += F[i + 1];
        if(i%step == 0){
            answer[(int)(i/step)] = sum;
            deaths[(int)(i/step)] = sum2;
        }
    }
